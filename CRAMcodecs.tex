...
\begin{algorithmic}[1]
\Function{RansDecodeX4}{$len$}
  \For{$i \gets 0$ to $3$} \Comment{Fetch 4 compressed lengths}
    \State $clen_i \gets $\Call{ReadUint7}{}
  \EndFor
  \State $X0 \gets $\Call{RansDecode4x16}{$len / 4$} \Comment{Decode 4 streams}
  \State $X1 \gets $\Call{RansDecode4x16}{$len / 4$}
  \State $X2 \gets $\Call{RansDecode4x16}{$len / 4$}
  \State $X3 \gets $\Call{RansDecode4x16}{$len / 4$}
  \Statex
  \State $i \gets 0$
  \For{$j \gets 0$ to $len / 4 - 1$} \Comment{Interleave}
...
\Function{RansDecode4x16}{$len$}
  ...
  \If{$flags \bitand$ \textsc{X4}}
    \State $data \gets $\Call{RansDecodeX4}{$len$}
    \State \Return $data$
  \EndIf{}
...
\Function{ArithDecode}{$len$}
  ...
  \EndIf
  \If{$flags \bitand$ \textsc{X4}}
    \State $data \gets $\Call{DecodeX4}{$len$}
    \State \Return $data$
  \EndIf{}
  \If{$flags \bitand$ \textsc{Pack}}
    \State $pack\_len \gets len$
...
\Function{DecodeX4}{$len$}
  % \State $ulen \gets $\Call{ReadUint7}{}
